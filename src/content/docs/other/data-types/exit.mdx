...
- `Exit.Failure` contains a failure [Cause](/other/data-types/cause) of type `E`.
...
An `Exit<A, E>` is conceptually an `Either<A, Cause<E>>`. However, it's important to note that [Cause](/other/data-types/cause) encompasses more states than just the expected error type `E`. It also includes other states such as interruption and defects (unexpected errors), as well as the possibility of combining multiple `Cause` values together.
