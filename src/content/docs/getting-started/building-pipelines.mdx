...
```ts twoslash file=<rootDir>/building-pipelines/pipe.ts
...
```ts twoslash file=<rootDir>/building-pipelines/map.ts
...
```ts twoslash file=<rootDir>/building-pipelines/as.ts
...
<Aside type="note">
  It's important to note that effects are **immutable**, meaning that when
  you use `Effect.flatMap` on an effect, it doesn't modify the original data
  type. Instead, it returns a new copy of the effect with the transformed
  value.
</Aside>
...
```ts twoslash file=<rootDir>/building-pipelines/flatMap.ts
...
```ts twoslash file=<rootDir>/building-pipelines/array-flattening.ts
...
```ts twoslash file=<rootDir>/building-pipelines/andThen.ts
...
```ts twoslash file=<rootDir>/building-pipelines/andThen-with-option.ts
...
```ts twoslash file=<rootDir>/building-pipelines/andThen-with-either.ts
...
```ts twoslash file=<rootDir>/building-pipelines/tap.ts
...
```ts twoslash file=<rootDir>/building-pipelines/all.ts
...
<Aside type="tip">
  ...
  head over to the [Introduction to Effect's Control Flow
  Operators](/getting-started/control-flow#all) documentation.
</Aside>
...
```ts twoslash file=<rootDir>/building-pipelines/pipeline.ts
...
```ts twoslash collapse={3-15} file=<rootDir>/building-pipelines/pipe-method.ts
