...
<Aside type="note" title="Optional Feature">
  ...
  Pipelines](/getting-started/building-pipelines) in Effect.
</Aside>
...
```ts file=<rootDir>/using-generators/program.ts
...
```ts file=<rootDir>/using-generators/gen.ts#L3-
...
```ts file=<rootDir>/using-generators/async-await.ts
...
```ts file=<rootDir>/using-generators/control-flow.ts
...
```ts file=<rootDir>/using-generators/raising-errors.ts
...
```ts file=<rootDir>/using-generators/short-circuiting.ts

```

<Aside type="note" title="Further Reading">
  If you want to dive deeper into error handling with Effect, you can explore
  the ["Error Management"](/error-management/two-error-types) section.
</Aside>
...
```ts file=<rootDir>/using-generators/passing-this.ts
...
```ts file=<rootDir>/using-generators/adapter.ts
