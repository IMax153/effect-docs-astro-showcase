...
<Aside type="note" title="Readonly Types by Default">
...
- `decodeUnknown`: Decodes a value and returns an [Effect](/getting-started/the-effect-type).
...
When your schema involves asynchronous transformations, neither the `decodeUnknownSync` nor the `decodeUnknownEither` functions will work for you. In such cases, you must turn to the `decodeUnknown` function, which returns an [Effect](/getting-started/the-effect-type).
...
<Aside type="tip" title="Options Impact">
  The [onExcessProperty](#excess-properties) and [error](#all-errors)
...
- `encode`: Encodes data and returns an [Effect](/getting-started/the-effect-type).
