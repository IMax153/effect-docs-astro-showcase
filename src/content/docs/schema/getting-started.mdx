...
<Aside type="note">
  ...
  `@effect/schema` return `readonly` types. For instance, in the `Person`
...
Alternatively, you can use the Class APIs (see the [Class APIs](/schema/classes) section for more details).
...
- `decodeUnknownOption`: Decodes a value and returns an [Option](/data-types/option) type.
- `decodeUnknownEither`: Decodes a value and returns an [Either](/data-types/either) type.
- `decodeUnknownPromise`: Decodes a value and returns a `Promise`.
- `decodeUnknown`: Decodes a value and returns an [Effect](/workflows/the-effect-type).
...
Now, let's see how to use the `decodeUnknownEither` function, which returns an [Either](/data-types/either) type representing success or failure.
...
Here, `ParseIssue` represents an error that might occur during the parsing process. It is wrapped in a tagged error to make it easier to catch errors using [Effect.catchTag](/guides/error-management/expected-errors#catchtag). The result `Either<A, ParseError>` contains the inferred data type described by the schema. A successful parse yields a `Right` value with the parsed data `A`, while a failed parse results in a `Left` value containing a `ParseError`.

When your schema involves asynchronous transformations, neither the `decodeUnknownSync` nor the `decodeUnknownEither` functions will work for you. In such cases, you must turn to the `decodeUnknown` function, which returns an [Effect](/workflows/the-effect-type).
...
- `encodeOption`: Encodes data and returns an [Option](/data-types/option) type.
- `encodeEither`: Encodes data and returns an [Either](/data-types/either) type representing success or failure.
- `encodePromise`: Encodes data and returns a `Promise`.
- `encode`: Encodes data and returns an [Effect](/workflows/the-effect-type).
...
Here is an example of a transformation that never fails during decoding. It returns an [Either](/data-types/either) containing either the decoded value or the original input. For encoding, it is reasonable to not support it and use `Forbidden` as the result.
...
- **Decoding**: The `SafeDecode` function ensures that decoding never fails. It wraps the decoded value in an [Either](/data-types/either), where a successful decoding results in a `Right` and a failed decoding results in a `Left` containing the original input.
