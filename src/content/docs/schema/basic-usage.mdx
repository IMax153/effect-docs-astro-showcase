...
<Aside type="tip" title="Effectful Filters">
  ...
  [filterEffect](/schema/transformations#effectful-filters).
</Aside>
...
<Aside type="note">
  ...
  [ArrayFormatter](/schema/error-formatters#arrayformatter).
</Aside>
...
- have already defined a [branded type](/code-style/branded-types) via `effect/Brand` and want to reuse it to define a schema
...
If you have already defined a [branded type](/code-style/branded-types) using the `effect/Brand` module, you can reuse it to define a schema using the `fromBrand` combinator exported by the `@effect/schema/Schema` module.
...
Here's how you can [transform](/schema/transformations#transform) the `Shape` schema into another schema that represents a discriminated union:
...
To improve the error message in case of failed decoding, remember to add [annotations](/schema/error-formatters#customizing-the-output):
