...
When working with schemas, you have a choice beyond the [Schema.Struct](/schema/basic-usage#structs) constructor.
...
- **Value Hashing and Equality**: Utilize the built-in capability for checking value equality and applying hashing (thanks to `Class` implementing [Data.Class](/data-types/data#class)).
...
Thanks to the implementation of [Data.Class](/data-types/data#class), instances of your classes automatically support the [Equal](/trait/equal) trait, which allows for easy comparison:
...
You can also create classes that extend [TaggedClass](/data-types/data#taggedclass) and [TaggedError](/data-types/data#taggederror) from the `effect/Data` module.
