...
To simplify, we can envision the Runtime System as a black box that takes both the effect [`Effect<A, E, R>`](/docs/getting-started/the-effect-type/) and its associated context `Context<R>`. It runs the effect and returns the result as an [`Exit<A, E>`](/docs/data-types/exit/) value.

![Effect Runtime](./_assets/runtime.svg "A diagram outlining how an Effect Runtime processes input and produces output")
...
5. **Ensure finalizers are run appropriately**. They have to ensure finalizers are run appropriately at the right point in all circumstances to make sure that resources are closed that clean-up logic is executed. This is the feature that powers [Scope](/docs/resource-management/scope/) and all the other resource-safe constructs in Effect.
