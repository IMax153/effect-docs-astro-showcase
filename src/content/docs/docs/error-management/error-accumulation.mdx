...
Sequential combinators such as [Effect.zip](/docs/getting-started/control-flow#zip) and [Effect.forEach](/docs/getting-started/control-flow#foreach) have a "fail fast" policy when it comes to error management. This means that they stop and return immediately when they encounter the first error.
...
<Aside type="caution" title="Lossy Function">
  Note that this function is lossy, which means that if there are any
  errors, all successes will be lost. If you need a function that
  preserves both successes and failures, please refer to the
