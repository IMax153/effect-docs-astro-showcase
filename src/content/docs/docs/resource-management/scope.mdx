...
//    ^? const runnable: Effect<number, never, never>
...
// Wrapping the effect in a scope
const runnable = Effect.scoped(program)
//    ^? const runnable: Effect<never, string, never>
...
**Example** (Adding a Finalizer on [Interruption](/docs/concurrency/interruption-model/))
...
// Wrapping the effect in a scope
const runnable = Effect.scoped(program)
//    ^? const runnable: Effect<never, never, never>
