...
The `take` sink takes the specified number of values from the stream and results in a [Chunk](/docs/data-types/chunk) data type.
...
To gather all the elements from a data stream into a [Chunk](/docs/data-types/chunk), you can employ the `Sink.collectAll()` function:
...
If you only want to collect a specific number of elements from a stream into a [Chunk](/docs/data-types/chunk), you can use `Sink.collectAllN(n)`:
