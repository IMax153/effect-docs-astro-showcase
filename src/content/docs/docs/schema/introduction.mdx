...
- **Context**. Similar to the [`Effect`](https://effect.website/docs/docs/getting-started/the-effect-type) type, it represents the contextual data required by the schema to execute both decoding and encoding. If this type parameter is `never` (default if not explicitly provided), it means the schema has no requirements.
...
![Encoding and Decoding](./encoding-and-decoding.svg "A sequence diagram which demonstrates the relationship between encoding and decoding in Effect Schema")
