...
Similar to [Effect.gen](/docs/getting-started/using-generators), there's also `Either.gen`, which provides a convenient syntax, akin to async/await, for writing code involving `Either` and using generators.
