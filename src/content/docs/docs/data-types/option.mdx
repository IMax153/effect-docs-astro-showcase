...
<Aside type="caution" title="Property Key Always Present">
...
<Aside type="tip" title="Matching">
  Using `Option.match` instead of `Option.isSome` or `Option.isNone` can
...
Similar to [Effect.gen](/docs/getting-started/using-generators), there's also `Option.gen`, which provides a convenient syntax, akin to async/await, for writing code involving `Option` and using generators.
