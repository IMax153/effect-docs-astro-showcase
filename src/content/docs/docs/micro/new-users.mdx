...
The `MicroExit` type is designed to capture the outcome of a `Micro` computation. It uses the [Either](/docs/data-types/either/) data type to distinguish between successful outcomes and failures:
...
const consoleLog = (message: string) =>
  Micro.sync(() => console.log(message))
...
const program1 = Micro.matchEffect(success, {
  onFailure: (error) =>
    Micro.succeed(`failure: ${error.message}`).pipe(
      Micro.tap(consoleLog)
    ),
  onSuccess: (value) =>
...
const program =
  ...
  Micro.scopeMake.pipe(
    // add finalizer 1
    Micro.tap((scope) =>
      scope.addFinalizer(() => consoleLog("finalizer 1"))
    ),
    // add finalizer 2
    Micro.tap((scope) =>
      scope.addFinalizer(() => consoleLog("finalizer 2"))
    ),
    // close the scope
    Micro.andThen((scope) =>
...
<Aside type="caution" title="Initial Execution Included">
