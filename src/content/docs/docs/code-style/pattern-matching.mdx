...
<Aside type="caution" title="Tag Field Naming Convention">
  Note that it only works with the convention within the Effect ecosystem
  of naming the tag field with `"_tag"`.
</Aside>
...
The `option` transformation returns the result encapsulated within an [Option](/docs/data-types/option). When the match succeeds, it represents the result as `Some`, and when there's no match, it signifies the absence of a value with `None`.
...
The `either` transformation might match a value, returning an [Either](/docs/data-types/either) following the format `Either<MatchResult, NoMatchResult>`.
