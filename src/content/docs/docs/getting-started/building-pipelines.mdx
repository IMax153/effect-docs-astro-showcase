...
![Effect Pipelines](../_assets/pipeline.svg "A flowchart displaying how functions are composed in an Effect pipeline")
...
It's worth noting that [Option](/docs/data-types/option/#interop-with-effect) and [Either](/docs/data-types/either/#interop-with-effect), types commonly used to handle optional values and simple error scenarios, are also compatible with `Effect.andThen`.
...
<Aside type="note" title="Option As Effect">
...
<Aside type="note" title="Either As Effect">
...
refer to the [Concurrency Options](/docs/concurrency/concurrency-options/)
documentation).
