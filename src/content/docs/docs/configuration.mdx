...
<Aside type="note">
  The `ConfigProvider.fromMap` function used in this example creates a
...
const serfviceConfig = Config.map(
  Config.all([
    Config.nested(config, "HOSTPORT"),
...
It parses the config value and wraps it in a `Redacted<string>`, a [data type](/docs/data-types/redacted/) designed for holding secrets.
...
Deprecated since version 3.3.0: Please use [Config.redacted](#redacted)
for handling sensitive information going forward.
...
const program = Config.secret("API_KEY").pipe(
  ...
  Effect.tap((secret) =>
    Console.log(`Secret value: ${Secret.value(secret)}`)
  )
)
