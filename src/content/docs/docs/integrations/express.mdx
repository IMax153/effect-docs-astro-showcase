...
4. Create a new file, for example, `hello-world.ts`, and add the following code:
   ...
   ```ts twoslash title="hello-world.ts"
...
- **Express Service**. We define an `Express` [service](/docs/requirements-management/services/) to retrieve the Express app later on.
...
- **Main Route**. The main route, `IndexRouteLive`, is defined as a [layer](/docs/requirements-management/layers/).
  ...
  We access the [runtime](/docs/runtime/) (`Effect.runtime`), which can be used to execute tasks within our route (`runFork`).
...
- **Server Setup**. The server is created in a layer (`ServerLive`) and mounted at the end of our program.
  ...
  We use [Effect.acquireRelease](/docs/resource-management/scope/#defining-resources) to create the server, allowing automatic management of the [scope](/docs/resource-management/scope/#scope).
...
// Express
//
// NB: this is an example of an integration to a third party lib,
// not the suggested way of integrating express
//
