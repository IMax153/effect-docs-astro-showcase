...
Another useful function for fibers is `Fiber.await`. This function returns an effect containing an [Exit](/docs/data-types/exit/) value, which provides detailed information about how the fiber completed.
...
Similar to `Fiber.await`, `Fiber.interrupt` returns an [Exit](/docs/data-types/exit/) value describing how the fiber completed.
...
**Note**: It is also possible to perform interruptions using the high-level API `Effect.interrupt`. For more information, see [Effect.interrupt](/docs/concurrency/interruption-model/#calling-effectinterrupt).
...
Effect provides many functions that accept [Concurrency Options](/docs/concurrency/concurrency-options/) to help you identify opportunities to parallelize your code.
...
If you need to handle the first effect to complete, whether it succeeds or fails, you can use the `Effect.either` function. This function wraps the result in an [Either](/docs/data-types/either/) type, allowing you to see if the outcome was a success (`Right`) or a failure (`Left`):
