...
```ts twoslash file=<rootDir>/services/Random.ts
...
<Aside type="note" title="Tag Identifiers">
...
```ts twoslash file=<rootDir>/services/using-Random-gen.ts
...
```ts twoslash file=<rootDir>/services/using-Random-pipe.ts
...
```ts twoslash file=<rootDir>/services/run-error.ts
...
To represent this optionality it returns an [Option](/data-types/option) of the implementation.
