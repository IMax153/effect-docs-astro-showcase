...
- `fiber`: The identifier of the [fiber](/guides/concurrency/fibers) executing the program.
...
For added context, you can also include one or more [Cause](/data-types/cause) instances in your logs,
...
This method confines the application of annotations to logs within a specific [Scope](/guides/resource-management/scope) of your effect computation:
...
To retrieve the log level from a [configuration](/guides/configuration) and incorporate it into your program, utilize the layer produced by `Logger.minimumLogLevel`:
...
To evaluate the configured program, you can utilize `ConfigProvider.fromMap` for testing (refer to [Testing Services](/guides/configuration#testing-services) for more details).
