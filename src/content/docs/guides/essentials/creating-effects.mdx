...
```ts twoslash file=<rootDir>/why-effect/divide.ts
...
```ts twoslash file=<rootDir>/creating-effects/succeed.ts
...
```ts twoslash file=<rootDir>/creating-effects/fail.ts
...
However, using "tagged" errors, which are objects with a `_tag` field, helps identify error types and integrates well with standard Effect functions like [Effect.catchTag](/guides/error-management/expected-errors#catchtag).

```ts twoslash file=<rootDir>/creating-effects/fail-with-custom-error.ts
...
```ts twoslash file=<rootDir>/creating-effects/fail-divide.ts
...
```ts twoslash file=<rootDir>/creating-effects/fail-user-retrival.ts
...
```ts twoslash file=<rootDir>/creating-effects/sync.ts
...
- **Execution**: The side effect (logging to the console) encapsulated within `program` won't occur until the effect is explicitly [run](/guides/essentials/running-effects). This allows you to define side effects at one point in your code and control when they are activated, improving manageability and predictability of side effects in larger applications.
...
You can think of it similar to an unexpected crash in the program, which can be further managed or logged using tools like [Effect.catchAllDefect](/guides/error-management/unexpected-errors#catchalldefect).
...
```ts twoslash file=<rootDir>/creating-effects/try.ts
...
```ts {6} twoslash file=<rootDir>/creating-effects/try-custom-error-handling.ts
...
```ts twoslash file=<rootDir>/creating-effects/promise.ts
...
**Handling Unexpected Errors**. If, despite precautions, the thunk passed to `Effect.promise` does reject, an `Effect` containing a ["defect"](/guides/error-management/unexpected-errors) is created, similar to what happens when using the [Effect.die](/guides/error-management/unexpected-errors#die--diemessage) function.
...
```ts twoslash file=<rootDir>/creating-effects/tryPromise.ts
...
```ts {7} twoslash file=<rootDir>/creating-effects/tryPromise-custom-error-handling.ts
...
```ts twoslash file=<rootDir>/creating-effects/async.ts
...
<Aside type="tip" title="Sync & Async Code">
...
```ts twoslash file=<rootDir>/creating-effects/suspend-lazy-evaluation.ts
...
<Aside type="tip">
  ...
  Effects](/guides/essentials/running-effects#runsync) for more details).
</Aside>
...
```ts twoslash file=<rootDir>/creating-effects/suspend-circular-dependencies.ts

```

<Aside type="tip">
  This example utilizes `Effect.zipWith` to combine the results of two effects
  (refer to the documentation on [zipping](/guides/control-flow#zipwith) for
  more details).
</Aside>
...
```ts twoslash file=<rootDir>/creating-effects/suspend-unifying-return-type.ts
...
Check out the next guide on [Running Effects](/guides/essentials/running-effects) to find out more.
