...
The module provides a single `KeyValueStore` [tag](/guides/context-management/services), which acts as the gateway for interacting with the store.
...
The module provides several built-in implementations of the `KeyValueStore` interface, available as [layers](/guides/context-management/layers), to suit different needs:
...
The `SchemaStore` interface allows you to validate and parse values according to a defined [schema](/guides/schema/introduction).
