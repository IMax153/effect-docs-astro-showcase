...
To simplify, we can envision the Runtime System as a black box that takes both the effect [Effect&lt;A, E, R&gt;](/guides/essentials/the-effect-type) and its associated context `Context<R>`. It runs the effect and returns the result as an [Exit&lt;A, E&gt;](/other/data-types/exit) value.
...
5. **Ensure finalizers are run appropriately**. They have to ensure finalizers are run appropriately at the right point in all circumstances to make sure that resources are closed that clean-up logic is executed. This is the feature that powers [Scope](/guides/resource-management/scope) and all the other resource-safe constructs in Effect.
