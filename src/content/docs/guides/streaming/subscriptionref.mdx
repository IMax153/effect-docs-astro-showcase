...
A `SubscriptionRef<A>` is a specialized form of a [SynchronizedRef](/guides/state-management/synchronizedref). It allows us to subscribe and receive updates on the current value and any changes made to that value.
