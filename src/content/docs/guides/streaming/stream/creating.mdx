...
In Effect, there are two essential asynchronous messaging data types: [Queue](/guides/concurrency/queue) and [PubSub](/guides/concurrency/pubsub). You can easily transform these data types into `Stream`s by utilizing `Stream.fromQueue` and `Stream.fromPubSub`, respectively.
