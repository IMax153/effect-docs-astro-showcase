...
<Aside type="note">
  ...
  [Schema.typeSchema](/guides/schema/projections#typeschema). This is because the
...
Both `decode` and `encode` functions not only receive the value to transform (`input`), but also the [parse options](/guides/schema/getting-started#parse-options) that the user sets when using the resulting schema, and the `ast`, which represents the low level definition of the schema you're transforming.
