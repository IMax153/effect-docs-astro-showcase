...
Alternatively, you can use the Class APIs (see the [Class APIs](/guides/schema/classes) section for more details).
...
- `decodeUnknown`: Decodes a value and returns an [Effect](/guides/essentials/the-effect-type).
...
When your schema involves asynchronous transformations, neither the `decodeUnknownSync` nor the `decodeUnknownEither` functions will work for you. In such cases, you must turn to the `decodeUnknown` function, which returns an [Effect](/guides/essentials/the-effect-type).
...
- `encode`: Encodes data and returns an [Effect](/guides/essentials/the-effect-type).
