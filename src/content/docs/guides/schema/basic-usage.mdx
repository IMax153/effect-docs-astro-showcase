...
<Aside type="note">
  For effectful filters, see [filterEffect](/guides/schema/transformations#effectful-filters).
</Aside>
...
<Aside type="note">
  ...
  [ArrayFormatter](/guides/schema/error-formatters#arrayformatter).
</Aside>
...
- have already defined a [branded type](/guides/style/branded-types) via `effect/Brand` and want to reuse it to define a schema
...
If you have already defined a [branded type](/guides/style/branded-types) using the `effect/Brand` module, you can reuse it to define a schema using the `fromBrand` combinator exported by the `@effect/schema/Schema` module.
...
Here's how you can [transform](/guides/schema/transformations#transform) the `Shape` schema into another schema that represents a discriminated union:
...
To improve the error message in case of failed decoding, remember to add [annotations](/guides/schema/error-formatters#customizing-the-output):
