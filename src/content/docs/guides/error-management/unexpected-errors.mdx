...
It's important to understand that `catchSomeDefect` can only handle defects, not [expected errors](/guides/error-management/expected-errors) (such as those caused by `Effect.fail`) or [interruptions](/guides/concurrency/interruption-model) in execution (such as when using `Effect.interrupt`).
