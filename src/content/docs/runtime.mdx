...
To simplify, we can envision the Runtime System as a black box that takes both the effect [`Effect<A, E, R>`](/getting-started/the-effect-type) and its associated context `Context<R>`. It runs the effect and returns the result as an [`Exit<A, E>`](/data-types/exit) value.
