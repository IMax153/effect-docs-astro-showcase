...
```ts "readonly _tag" showLineNumbers=false
class HttpError {
...
```ts collapse={1-21} {29-31} file=<rootDir>/expected-errors/catch-some-if-all.ts
...
```ts collapse={1-21} {25} file=<rootDir>/expected-errors/catchIf-guard.ts
...
```ts collapse={1-21} {27-29} file=<rootDir>/expected-errors/catchTag-all.ts
...
<Aside type="caution">
  ...
  `readonly _tag` discriminant field. this field is required for the
  matching and handling of specific error tags.
</Aside>
