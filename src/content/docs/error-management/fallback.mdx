...
```ts file=<rootDir>/fallback/orElse.ts
...
```ts file=<rootDir>/fallback/orElseFail.ts
...
```ts file=<rootDir>/fallback/orElseSucceed.ts
...
```ts file=<rootDir>/fallback/firstSuccessOf.ts
