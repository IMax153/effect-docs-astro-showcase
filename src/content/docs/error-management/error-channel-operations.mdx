...
Similar to [tapping](/getting-started/using-pipelines#tap) for success values, Effect provides several operators for **inspecting error values**. These operators allow developers to observe failures or underlying issues without modifying the outcome:
