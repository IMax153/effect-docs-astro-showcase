...
```ts file=<rootDir>/timeout/timeout.ts
...
```ts file=<rootDir>/timeout/timeoutOption.ts
...
1. **Interruptible Operation**: If the operation can be interrupted, it is terminated immediately once the timeout threshold is reached, resulting in a `TimeoutException`.
   ...
   ```ts file=<rootDir>/timeout/interruptible.ts
...
2. **Uninterruptible Operation**: If the operation is uninterruptible, it continues until completion before the `TimeoutException` is assessed.
   ...
   ```ts file=<rootDir>/timeout/uninterruptible.ts
...
```ts file=<rootDir>/timeout/disconnect.ts
...
```ts file=<rootDir>/timeout/timeoutFail.ts
...
```ts file=<rootDir>/timeout/timeoutFailCause.ts
...
```ts file=<rootDir>/timeout/timeoutTo.ts
