...
- [Exit](/data-types/exit) is the preferred **result type** within Effect for capturing comprehensive details about failures.
...
<Aside type="tip" title="Matching">
  Using `Either.match` instead of `Either.isLeft` or `Either.isRight` can
  be more expressive and provide a clear way to handle both cases of an
  `Either`.
</Aside>
...
const foo = Effect.runSync(
  Effect.andThen(Effect.succeed([1, 2, 3]), head)
)
...
Similar to [Effect.gen](/getting-started/using-generators), there's also `Either.gen`, which provides a convenient syntax, akin to async/await, for writing code involving `Either` and using generators.
