...
<Aside type="note" title="Precision">
  ...
  Types](/code-style/branded-types)). Additionally, you may want to include
  more detailed information in the errors.
</Aside>
...
<Aside type="note">
  ...
  return the expected data - for this, you can use `@effect/schema` or
...
It's crucial for the requests to be modeled in a way that allows them to be comparable. This means implementing comparability (using methods like [Equals.equals](/trait/equal)) to identify and batch identical requests effectively.
...
<Aside type="note">
  ...
  are many different ways to create resolvers. For further details, consider
  ...
  Effect.gen(function* () {
    ...
    const resolver = RequestResolver.fromEffect(
      (request: Requests.GetTodos) =>
        Effect.tryPromise<Array<Todo>, GetTodosError>({
          try: () =>
            http
              .fetch("https://api.example.demo/todos")
              .then((res) => res.json()),
