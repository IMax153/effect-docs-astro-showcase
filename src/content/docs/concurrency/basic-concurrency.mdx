...
**Note**: It is also possible to perform interruptions using the high-level API `Effect.interrupt`. For more information, see [Effect.interrupt](/concurrency/interruption-model#calling-effectinterrupt).
...
Effect provides many functions that accept [Concurrency Options](/concurrency/concurrency-options) to help you identify opportunities to parallelize your code.
