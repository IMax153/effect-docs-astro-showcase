...
```ts twoslash file=<rootDir>/running-effects/runSync.ts
...
```ts twoslash file=<rootDir>/running-effects/runSync-caution.ts
...
```ts twoslash file=<rootDir>/running-effects/runSyncExit.ts
...
```ts twoslash file=<rootDir>/running-effects/runSyncExit-caution.ts
...
```ts twoslash file=<rootDir>/running-effects/runPromise.ts

```

`Effect.runPromise` will reject with an error if your Effect fails

```ts twoslash file=<rootDir>/running-effects/runPromise-caution.ts
...
```ts twoslash file=<rootDir>/running-effects/runPromiseExit.ts
...
```ts twoslash file=<rootDir>/running-effects/runFork.ts

```

In this example, the `program` continuously logs "running..." with each repetition spaced 200 milliseconds apart. You can learn more about repetitions and scheduling in our [Introduction to Scheduling](/guides/scheduling/introduction) guide.
...
For a deeper understanding of how fibers work and how to handle interruptions, check out our guides on [Fibers](/guides/concurrency/fibers) and [Interruptions](/guides/concurrency/interruption-model).
